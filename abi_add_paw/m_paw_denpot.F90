...
!  Allocations of "on-site" densities
   ...
   if (ipositron/=0) then ! Additional allocation for the electron-positron case
     LIBPAW_ALLOCATE(rho1_ep ,(cplex*mesh_size,lm_size,nspden))
     LIBPAW_ALLOCATE(trho1_ep,(cplex*mesh_size,lm_size,nspden))
     LIBPAW_ALLOCATE(nhat1_ep,(cplex*mesh_size,lm_size,nspden*usenhat))
   end if
...
!     LIBPAW_ALLOCATE(lmselect_ep,(lm_size))
!     LIBPAW_ALLOCATE(lmselect_cur_ep,(lm_size))
...
!     LIBPAW_DEALLOCATE(lmselect_ep)
!     LIBPAW_DEALLOCATE(lmselect_cur_ep)
!   end if
...
!  ======================================================================
! Not supporting hybrid functional for now
!   if (pawtab(itypat)%useexexch/=0.and.ipert==0.and.ipositron/=1) then
!
...
!    Positron special case (dij0 is opposite, except for kinetic term)
     if (ipositron==1) then
       LIBPAW_ALLOCATE(dij_ep,(lmn2_size))
...
!  Deallocate densities
   ...
   if (ipositron/=0)  then
     LIBPAW_DEALLOCATE(rho1_ep)
     LIBPAW_DEALLOCATE(trho1_ep)
     LIBPAW_DEALLOCATE(nhat1_ep)
   end if
...
!     end if
     ...
     call xpaw_mpi_sum(mpiarr,my_comm_atom,ierr)
