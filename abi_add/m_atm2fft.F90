...
! integer,intent(in) :: atindx1(natom),nattyp(ntypat),ngfft(18),qprtrb(3)
 integer,intent(in) :: atindx1(natom),nattyp(ntypat),ngfft(3),qprtrb(3)
 ...
 !distribfft is replaced by directly supplying the two integer arrays
 !see also the modified m_paw_nhat.F90 in this interface
 !the G space arrays are distributed along the 2nd dimension
...
!arrays
 !integer, LIBPAW_CONTIGUOUS pointer :: fftn2_distrib(:),ffti2_local(:)
 real(dp), LIBPAW_CONTIGUOUS pointer :: tvalespl(:,:),tcorespl(:,:),ttaucorespl(:,:)
...
!Get the distrib associated with this fft_grid
 ...
 !else
 !  LIBPAW_BUG("Unable to find an allocated distrib for this fft grid")
 !end if

 !is not supplied in ABINIT
 if (present(is2_in)) then
   if(is2_in<1.or.is2_in>6) then
     LIBPAW_BUG("is2_in must be between 1 and 6")
   else
     ndir = 1
   end if
   ndir = -1
...
!Zero out arrays to permit accumulation over atom types
 if (optv==1.and.optatm==1) then
   LIBPAW_ALLOCATE(workv,(2,nfft))
   workv(:,:)=zero
 end if
 if (optn==1.and.optatm==1) then
   LIBPAW_ALLOCATE(workn,(2,nfft))
   workn(:,:)=zero
 end if
 if (optv==1.and.optgr==1) then
   LIBPAW_ALLOCATE(grv_indx,(3,natom))
   grv_indx(:,:)=zero
 end if
 if (optn==1.and.optgr==1) then
   LIBPAW_ALLOCATE(grn_indx,(3,natom))
   grn_indx(:,:)=zero
 end if
 if (optv==1.and.optdyfr==1) then
   LIBPAW_ALLOCATE(dyfrv_indx,(3,3,natom))
   dyfrv_indx(:,:,:)=zero
 end if
 if (optn==1.and.optdyfr==1) then
   LIBPAW_ALLOCATE(dyfrn_indx,(3,3,natom))
   dyfrn_indx(:,:,:)=zero
 end if
...
&         +gprimd(ka,:)*gprimd(kg,jj)+gprimd(kg,:)*gprimd(ka,jj)
 ...
 LIBPAW_ALLOCATE(phre_igia,(natom))
 LIBPAW_ALLOCATE(phim_igia,(natom))

 !Initialize Gcut-off array from m_termcutoff
 !In ABINIT, icutcoul = 3 is used, which corresponds to
 !no cutoff applied (i.e. gcutoff == 1.0)

 !LIBPAW_ALLOCATE(gcutoff,(nfft))
...
!  ia1,ia2 sets range of loop over atoms:
   ...
   if (usepaw == 1) then
     ...
     message = 'Please use this for PAW'
     LIBPAW_BUG(message)
...
!  End loop on type of atoms
 ...
 LIBPAW_DEALLOCATE(phre_igia)
 LIBPAW_DEALLOCATE(phim_igia)
 !LIBPAW_DEALLOCATE(gcutoff)
...
!Additional treatment in case of parallelization
 if (present(comm_fft)) then
   if ((xpaw_mpi_comm_size(comm_fft)>1).and.&
&   (optgr==1.or.optstr==1.or.optdyfr==1.or.opteltfr==1)) then
     !call timab(48,1,tsec)
     if (optv==1) then
       if (optgr==1)then
         call xpaw_mpi_sum(grv_indx,comm_fft,ierr)
       end if
       if (optstr==1)then
         call xpaw_mpi_sum(strv,comm_fft,ierr)
       end if
       if (optdyfr==1)then
         call xpaw_mpi_sum(dyfrv_indx,comm_fft,ierr)
       end if
     end if
     if (optn==1) then
       if (optgr==1)then
         call xpaw_mpi_sum(grn_indx,comm_fft,ierr)
       end if
       if (optstr==1)then
         call xpaw_mpi_sum(strn,comm_fft,ierr)
       end if
       if (optdyfr==1)then
         call xpaw_mpi_sum(dyfrn_indx,comm_fft,ierr)
       end if
     end if
...
!Forces: re-order atoms
 if (optgr==1) then
   if (optv==1) then
     ...
     LIBPAW_DEALLOCATE(grv_indx)
   end if
   if (optn==1) then
     do ia=1,natom
       grn(1:3,atindx1(ia))=grn_indx(1:3,ia)
     end do
     LIBPAW_DEALLOCATE(grn_indx)
   end if
 end if
...
!Dynamical matrix: re-order atoms
 if (optdyfr==1) then
   if (optv==1) then
     ...
     LIBPAW_DEALLOCATE(dyfrv_indx)
   end if
   if (optn==1) then
     do ia=1,natom
       dyfrn(1:3,1:3,atindx1(ia))=dyfrn_indx(1:3,1:3,ia)
     end do
     LIBPAW_DEALLOCATE(dyfrn_indx)
   end if
 end if
